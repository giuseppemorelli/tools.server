...
if [ ! -d $BACKUP_DIR ]
  then
    ...
    if [ $EMAIL -eq 1 ]
      then
      echo "WARNING!!! The specified backup directory $BACKUP_DIR does not exist. Operation canceled." | mail -s "$EMAILSUBJECT" $EMAILADDRESS
    fi
    echo "WARNING!!! The specified backup directory $BACKUP_DIR does not exist. Operation canceled."
    exit 1
fi
...
if test $DAYOFWEEK -eq $LEVEL0DAY
  then
    ...
    do
      ...
      OUTFILENAME=$WITHOUTLEADINGDOT.`date +"%Y%m%d_%s"`.tar
